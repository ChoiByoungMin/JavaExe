...
/*
 * ShowOwnNameThread는 이미 OwnName을 상속받았으므로
 * Thread의 상속을 받을 수 없다.
...
public class ImplementsRunnable {
	public static void main(String[] args) {
		...
		ShowOwnNameThread s3 = new ShowOwnNameThread("바둑이");
		ShowOwnNameThread s4 = new ShowOwnNameThread("고트");
		
		// 스레드를 동작(run() 을 호출)
		System.out.println("s1 스레드 객체 동작");
		Thread t1 = new Thread(s1);t1.start();
		System.out.println("s2 스레드 객체 동작");
		Thread t2 = new Thread(s2);t2.start();
		System.out.println("s3 스레드 객체 동작");
		Thread t3 = new Thread(s3);t3.start();
		System.out.println("s4 스레드 객체 동작");
